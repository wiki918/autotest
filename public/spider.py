...
class Spider:
    ...
    def get_html_data(self):
        ...
        statuses = []
        if self.bsobj:
            test_stephtmls = self.bsobj.find_all('div', {"class": "testcase"})
            for i in test_stephtmls:
                test_cases.append(i.get_text())
                #pass
                if i.parent.attrs['class'][0]=="none":
                    statuses.append(1)
                #500的用例
                elif i.parent.attrs['class'][0]=="errorCase":
                    statuses.append(0)
                #断言错误的用例
                elif i.parent.attrs['class'][0] == "failCase":
                    statuses.append(2)
